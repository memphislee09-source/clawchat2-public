...
- refined contacts list styling with synced agent avatars, a continuous list layout, and single-divider row separation
- refined chat message presentation with speaker/time metadata above each bubble at the top-left and synced user display names from `openclaw-webchat`
...
- single-line plain-text bubbles shrink to content width, while multi-line text and attachment-bearing bubbles keep the wider conversation width
...
- inline chat video cards now show a first-frame preview instead of a generic black placeholder when the video source is reachable
- agent-sent images, audio, videos, and regular files can now be downloaded into Android public storage from the chat UI
...
- inline streamed chat videos now reuse the playable source to extract a first-frame preview before fullscreen playback
- downloaded agent attachments now save into Android public media/download collections instead of remaining trapped inside app-private cache
...
- contacts now use a cleaner WeChat-style list treatment instead of boxed rows, with larger avatars and exactly one subtle divider between adjacent rows
...
- chat bubbles now place speaker and timestamp outside the bubble at the top-left while keeping the denser left-aligned transcript layout
...
- current public release page: `https://github.com/memphislee09-source/clawchat2-public/releases/tag/v0.2.7`
- current public release asset: `openclaw-0.2.7-play-release.apk`
- current public release SHA256: `39d90b1ce627ec52b2e93a63b854dcb87a8706b81018d98a438431cad6e5bdcd`
- current development build version is now `0.2.8`
...
- attachment download support is now part of the promoted release baseline, covering image/audio/video/file saves into `ClawChat2` public folders on Android
...
- 联系人列表样式已继续细化，包含同步 agent 头像、连续列表布局，以及相邻联系人之间仅保留一条更浅的分隔线
- 聊天消息展示也已调整为将发言人与时间移到气泡外左上角，并同步 `openclaw-webchat` 中“我”的名称
...
- 单行纯文本消息气泡现在会按内容宽度收缩；多行文本消息和带附件消息继续保持更宽的对话宽度
...
- 聊天中的视频卡片现在会在视频源可达时显示首帧预览，不再统一落到黑色占位块
...
- 聊天中的串流视频现在会复用可播放视频源提取首帧预览，再进入全屏播放
- 已下载的 agent 附件现在会保存进 Android 公共媒体/下载目录，不再只停留在 app 私有缓存里
...
- 联系人列表现已改为更接近微信通讯录的连续列表样式，头像更大、相邻联系人之间只保留一条更轻的分隔线，不再使用外框卡片
- 聊天页现在也延续相同的轻量交互方向，并会把用户发言标题与 `openclaw-webchat` 里的“我”名称同步
- 聊天气泡中的发言人与时间现在移到气泡外左上角显示，同时保留更紧凑的对话密度
...
- 当前公开 release 页面：`https://github.com/memphislee09-source/clawchat2-public/releases/tag/v0.2.7`
- 当前公开 release 资产：`openclaw-0.2.7-play-release.apk`
- 当前公开 release SHA256：`39d90b1ce627ec52b2e93a63b854dcb87a8706b81018d98a438431cad6e5bdcd`
- 当前开发构建版本已提升到 `0.2.8`
- 当前公开仓首页已加入 Bilibili 介绍视频链接，并用新的单张合成预览图替换旧截图组
- 当前这条发布基线已同时包含配对重试抑制修复与首次安装语言选择入口
- 当前这条发布基线也已包含附件下载能力，覆盖图片、音频、视频与普通文件的保存到手机路径
