...
- refined chat message presentation with denser in-bubble speaker/time headers and synced user display names from `openclaw-webchat`
...
- chat bubbles now place speaker and timestamp back on the first in-bubble row while keeping the denser left-aligned transcript layout
...
- current development build version is now `0.2.7`
...
- 聊天消息展示也已调整为更紧凑的气泡内标题布局，并同步 `openclaw-webchat` 中“我”的名称
...
- 聊天气泡中的发言人与时间现在回到气泡第一行显示，保留更紧凑的对话密度
